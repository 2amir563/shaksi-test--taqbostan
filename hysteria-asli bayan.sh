...
# ------------------ بخش لینک‌های اختصاصی بیان شما ------------------
# این لینک‌ها را با لینک‌های مستقیم بیان خودت جایگزین کن
HYSTERIA_VERSION_AMD64="https://bayanbox.ir/download/7942099860131521513/hysteria-linux-amd64"
MONITOR_PY_URL="https://bayanbox.ir/download/3872541556002887293/hysteria-monitor.py"

# ------------------ Initialization & Core Download ------------------
ARCH=$(uname -m)
case "$ARCH" in
  x86_64)   DOWNLOAD_URL="$HYSTERIA_VERSION_AMD64" ;;
  *) colorEcho "Architecture $ARCH not supported." red; exit 1 ;;
esac

if [ ! -f "/usr/local/bin/hysteria" ]; then
  colorEcho "Downloading Hysteria binary from Bayan..." cyan
  curl -fsSL "$DOWNLOAD_URL" -o hysteria
  chmod +x hysteria
...
if [ ! -f /etc/hysteria/hysteria-monitor.py ]; then
  colorEcho "Downloading Monitoring script from Bayan..." cyan
